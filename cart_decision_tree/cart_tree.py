    ...
    def alpha_list_classification(alpha, tree, data_set, features_name):
        ...
        for value in list(tree_new[node_feature].keys()):  # 寻找非叶结点并划分数据集
            if type(tree_new[node_feature][value]) == dict and value != '否':
                alpha = CartTree.alpha_list_classification(alpha, tree_new[node_feature][value], data_set,
                                                           feat_name_new)  # 对左支递归
                if type(tree_new[node_feature]['否']) == dict:
                    alpha = CartTree.alpha_list_classification(alpha, tree_new[node_feature]['否'], data_set,
                                                               feat_name_new)  # 对右支递归
            ...
            if key != '否':
                if type(tree[node_feature][key]) == dict or type(tree[node_feature]['否']) == dict:  # 左支或右支为子树
                    if type(tree[node_feature][key]) == dict:  # 左支是子树，对左支递归
                        alpha = CartTree.alpha_list_regression(alpha, tree[node_feature][key], data_set,
                                                               feat_name_new[:])
                    if type(tree[node_feature]['否']) == dict:  # 右支是子树，对右支递归
                        alpha = CartTree.alpha_list_regression(alpha, tree[node_feature]['否'], data_set,
                                                               feat_name_new[:])
        return alpha
